...
WORKDIR /workspace
RUN git clone https://github.com/ebloc/ebloc-workflow-engine.git
WORKDIR /workspace/ebloc-workflow-engine
...
## Finally
WORKDIR /workspace/ebloc-workflow-engine/workflow-engine
RUN apt-get clean \
 && apt-get autoremove \
 && apt-get autoclean \
 && ipfs version \
